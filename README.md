...
```mermaid
graph TD
    A[IPジオルーティングによるユーザーエントリー] -- B{所在地？}
...
graph TD
    ...
    C --> G[ステークでフィルタ<br/>プレイヤー：2～9<br/>スピード]
    D --> H[スケジュールと<br/>フォーマット確認]
    E --> I[招待コードで<br/>作成または参加]
    F --> J[練習モード<br/>STARのみ]
    
    G --> K[テーブル選択<br/>2～9プレイヤー]
    H --> K
    I --> K
    J --> K
    
    K --> L{席空き？}
    L -->|あり| M[テーブル参加<br/>最大2～9プレイヤー]
    L -->|なし| N[ウェイトリスト<br/>または自動着席]
    
    M --> O[バイイン画面]
    O --> P{モデル？}
    P -->|リアルマネー| Q[QまたはSTARで<br/>支払い]
    P -->|スイープステークス| R[STARまたはQs<br/>使用]
    
    Q --> S[バイイン確認]
    R --> S
    S --> T[ゲーム参加<br/>テーブル：2～9プレイヤー<br/>チャット＋サポート有効]
    
    T --> U[ゲームプレイループ]
    U --> V[2～9プレイヤーに<br/>ハンド配布]
    V --> W[ベッティングラウンド<br/>15～30秒時計]
    W --> X{全アクション完了？}
    ...
    CHAT --> SEND[メッセージ送信"<br/>"絵文字、@メンション、ミュート]
    SEND --> MOD[自動モデレーション"<br/>"不適切語フィルタ、スパムブロック]
    MOD --> DISPLAY[メッセージ表示<br/>ピン留めディーラーノート]
    
    %% サポート統合
    T --> SUPPORT[サポートボタン<br/>ライブヘルプまたはFAQ]
    SUPPORT --> TICKET[サポートチケット作成<br/>スクリーンショット添付]
    TICKET --> RESPONSE[エージェント応答<br/>ゲーム内チャットまたはメール]
    RESPONSE --> RESOLVED[問題解決<br/>プレイ再開]
    
    %% プレイヤーアクション
    BB --> CC{プレイヤーアクション？}
    CC -->|シットアウト| DD[自動フォールド<br/>復帰まで]
    CC -->|リバイ| EE[チップ追加<br/>最小/最大制限]
    CC -->|テーブル退出| FF[キャッシュアウト<br/>または没収]
    CC -->|チャット| CHAT
    CC -->|ヘルプ| SUPPORT
