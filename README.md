...
# ポーカールームの概要フローチャート  
**2–9プレイヤー • ゲーム内チャット • ライブサポート**

```mermaid
graph TD
    A[ユーザーがロビーに入る] -- B{ルームタイプを選択}
    ...
    M --> O[バイイン画面]
    O --> P{モード？}
    P -->|リアルマネー| Q[QまたはSTARで\n支払い]
    P -->|スイープステークス| R[STARまたはQsを\n使用]
    ...
    CHAT --> SEND[メッセージを送信\n絵文字、@メンション、ミュート]
    SEND --> MOD[自動モデレーション\n不適切な言葉フィルター、スパムブロック]
    MOD --> DISPLAY[メッセージが表示\nピン留めされたディーラーノート]
    
    %% サポート統合
    T --> SUPPORT[サポートボタン\nライブヘルプまたはFAQ]
    SUPPORT --> TICKET[サポートチケットを開く\nスクリーンショットを添付]
    TICKET --> RESPONSE[エージェントの応答\nゲーム内チャットまたはメール]
    RESPONSE --> RESOLVED[問題解決\nプレイを再開]
    
    %% プレイヤーアクション
    BB --> CC{プレイヤーのアクション？}
    CC -->|シットアウト| DD[自動フォールド\n復帰まで]
    CC -->|リバイ| EE[チップを追加\n最小/最大制限]
    CC -->|テーブルを離れる| FF[キャッシュアウト\nまたは放棄]
    CC -->|チャット| CHAT
    CC -->|ヘルプ| SUPPORT
...
graph TD
    ...
    C --> G[ステークスでフィルター<br/>プレイヤー: 2–9<br/>スピード]
    D --> H[スケジュール<br/>とフォーマットを表示]
    E --> I[作成または<br/>招待コードで参加]
    F --> J[練習モード<br/>STARのみ]
    
    G --> K[テーブルを選択<br/>2–9プレイヤー]
    H --> K
    I --> K
    J --> K
    
    K --> L{席は空いていますか？}
    L -->|はい| M[テーブルに参加<br/>最大2–9プレイヤー]
    L -->|いいえ| N[待機リスト<br/>または自動着席]
    
    M --> O[バイイン画面]
    O --> P{モード？}
    P -->|リアルマネー| Q[QまたはSTARで<br/>支払い]
    P -->|スイープステークス| R[STARまたはQsを<br/>使用]
    
    Q --> S[バイインを確認]
    R --> S
    S --> T[ゲームに参加<br/>テーブル: 2–9プレイヤー<br/>チャット + サポート有効]
    
    T --> U[ゲームプレイのループ]
    U --> V[2–9プレイヤーに<br/>ハンドが配られる]
    V --> W[ベッティングラウンド<br/>15–30秒の時計]
    W --> X{すべてのアクションが完了？}
    X -->|はい| Y[次のストリート<br/>またはショーダウン]
    X -->|いいえ| W
    Y --> Z[ポットが授与される]
    Z --> AA[残高を更新<br/>リアルタイム]
    AA --> BB[新しいハンド<br/>自動配布]
    
    %% チャット統合
    T --> CHAT[ゲーム内チャットパネル<br/>テーブルチャット + ディーラーメッセージ]
    CHAT --> SEND[メッセージを送信<br/>絵文字、@メンション、ミュート]
    SEND --> MOD[自動モデレーション<br/>不適切な言葉フィルター、スパムブロック]
    MOD --> DISPLAY[メッセージが表示<br/>ピン留めされたディーラーノート]
    
    %% サポート統合
    T --> SUPPORT[サポートボタン<br/>ライブヘルプまたはFAQ]
    SUPPORT --> TICKET[サポートチケットを開く<br/>スクリーンショットを添付]
    TICKET --> RESPONSE[エージェントの応答<br/>ゲーム内チャットまたはメール]
    RESPONSE --> RESOLVED[問題解決<br/>プレイを再開]
    
    %% プレイヤーアクション
    BB --> CC{プレイヤーのアクション？}
    CC -->|シットアウト| DD[自動フォールド<br/>復帰まで]
    CC -->|リバイ| EE[チップを追加<br/>最小/最大制限]
    CC -->|テーブルを離れる| FF[キャッシュアウト<br/>または放棄]
    CC -->|チャット| CHAT
    CC -->|ヘルプ| SUPPORT
...
| 機能 | 実装 |
|------|------|
| テーブルあたりのプレイヤー数 | 2～9（ヘッズアップからフルリング） |
| ゲーム内チャット | リアルタイムテーブルチャット、絵文字、@メンション、ミュート、ディーラーメッセージ、自動モデレーション |
| ライブサポート | ゲーム内サポートボタン → スクリーンショット付きチケット → エージェントがチャット/メールで応答 |
| ゲームフロー | 自動ハンド配布、15～30秒アクション時計、リバイ、シットアウト、退出 |
| クロスモデル | リアルマネー＆スイープステークスで同一フロー — 通貨のみ異なる |
| 括弧やパイプなし | 全てのラベルはクリーンでMermaid対応 |
| 暗いテキスト＋明るい背景 | 標準Mermaidレンダリングで保証 |

## ゲーム発売ポータル（フェーズ2）

```mermaid
graph TD
    A[フェーズ2：ゲーム投稿] --> B[ゲームアイデア投稿<br/>コスト：1K STAR または 50 Qs]
    B --> C[投票サイクル：月～日 UTC<br/>閾値：50.01%<br/>再投票可能]
    C --> D{可決？}
    D -->|はい| E[ライブフロアに追加]
    D -->|いいえ| F[ゲームアーカイブ]
    E --> G[監視：売上、バグ、18ヶ月以上]
    G --> H{終了？}
    H -->|はい| I[投票 → 1週間猶予 → アーカイブ/バーン]
    H -->|いいえ| E
    I --> J[デュアルガバナンス：共有投票<br/>米国投票はソーシャルゲームに重み]
```
