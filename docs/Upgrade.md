...
The upgrade does five tightly coupled things:
...
3. Runs Coin Flip behind a **14-epoch beta window** for **eligible QST holders only**. After `betaStartEpoch` is anchored (first `BEGIN_EPOCH` once this code is live), only identities possessing at least **`QUSINO_BETA_MIN_QST_HOLDING` (100,000 QST)** may call `coinFlip`. When those 14 epochs elapse, the gate turns off and anyone may play.
4. **Removes epoch-revenue dividends to QST holders.** QST is no longer a revenue-share instrument. It is used **solely as a Membership / VIP token** (tiered access, beta eligibility, perks; sale/redemption inventory may remain).
5. **Reassigns the former 30% QST slice of `epochRevenue` to Qusino SC shares.** `qpi.distributeDividends` receives that 30% in addition to the existing 20% shareholder slice, so SC shareholders receive **50%** of snapshotted epoch revenue. LP (20%), CCF (5%), and treasury (25%) are unchanged.

**Why the QST change.** QST dividends created a yield / investment profile on a token that also gates a chance-game platform. To keep Qusino operable under the gambling and securities rules of the **targeted jurisdictions**, QST must not pay protocol dividends. Membership / VIP utility only — including the time-limited beta play gate — is the intended legal posture. This proposal does not give legal advice; it records that product constraint as the governance reason for the cut.

The Coin Flip / Result Bank / beta-gate change is implemented in [qubic/core PR #998](https://github.com/qubic/core/pull/998) (`src/contracts/Qusino.h`, tests in `test/contract_qusino.cpp`). Because the contract state struct changes, `contract_def.h` registers a **RESET** for `QUSINO_CONTRACT_INDEX` (not PADDING): `{ QUSINO_CONTRACT_INDEX, RESET, 232 }`. RESET discards the saved Qusino state and zero-fills the buffer. Core review has approved the integration shape; because contract **logic** (and state handling) change, quorum approval is required before merge / inclusion.
...
The first 14 epochs after go-live are a **membership beta**: only wallets that already hold enough QST can play. That is how QST is used as VIP / access, not as a yield token.
...
- keeps QST as **Membership / VIP only** (Player’s Card / tiered perks; 14-epoch beta play gate; optional buy/sell inventory);
...
9. **QST = membership only** — no QST holder dividend; QST utility is access / VIP, including the beta play gate, not a yield claim.
10. **14-epoch QST beta** — Coin Flip is restricted to eligible QST holders for `QUSINO_BETA_DURATION_EPOCHS` (14) after `betaStartEpoch` is anchored; then the gate expires.
11. **Jurisdiction-safe token shape** — QST must not look like a dividend security in targeted markets; the 30% revenue slice moves to SC shares instead.
...
| Piece | Behavior |
| --- | --- |
| `coinFlip(guess, assetType, amount)` | Guess heads or tails; `assetType` is QSC or STAR; `amount` ≥ minimum bet. During beta, caller must also pass the QST holding gate (see 4.3). |
...
### 4.3 Beta duration — 14 epochs, eligible QST holders only

Coin Flip is not open to every wallet on day one. The PR adds a membership beta:

| Piece | Value / behavior |
| --- | --- |
| Duration | `QUSINO_BETA_DURATION_EPOCHS` = **14** |
| Minimum QST | `QUSINO_BETA_MIN_QST_HOLDING` = **100,000** QST |
| Anchor | `betaStartEpoch` starts at 0. First `BEGIN_EPOCH` after this code is live sets `betaStartEpoch = qpi.epoch()`. |
| Gate | While `betaStartEpoch != 0` and `qpi.epoch() < betaStartEpoch + 14`, `coinFlip` requires the invocator to possess ≥ 100,000 QST on **QX** (`numberOfPossessedShares` with `QX_CONTRACT_INDEX`, not Qusino as managing contract). |
| Failure | `QUSINO_INSUFFICIENT_QST_FOR_BETA` (log `QUSINO_LOG_INSUFFICIENT_QST_FOR_BETA`). |
| After beta | Anyone may call `coinFlip` regardless of QST holdings. |
| Frontend view | `getUserAssetVolume` also returns `betaActive`, `betaEligible`, and `betaEpochsRemaining` so UIs do not reimplement the gate. |

QST is a QX-issued asset. Management rights are not transferred to Qusino, so the gate **must** query QX possession. Querying Qusino as manager would read 0 for every real holder.

This is the on-contract meaning of “Membership / VIP”: QST holdings unlock play during beta. It is **not** a dividend. After 14 epochs the play restriction ends; QST can still be used off-contract for perks.

### 4.4 Bankroll (`bonusAmount`)
...
### 4.5 QST: no dividends, Membership / VIP only
...
**After.** That loop is **removed**. QST possessors receive **zero** epoch-revenue Qu from Qusino. `QUSINO_QST_HOLDERS_DIVIDENDS_PERCENT` is unused / gone. QST is still read for the beta holding check.

**QST that continues:**

- Membership / VIP: holdings remain the on-chain signal for Player’s Card / tiered perks.
- 14-epoch Coin Flip beta gate (≥ 100,000 QST).
...
### 4.6 Epoch revenue split (30% moves to SC shares)
...
- `QUSINO_INSUFFICIENT_QST_FOR_BETA`
...
Qusino uses **RESET**. Pre-upgrade on-contract Qusino state is **not** migrated field-by-field. It is wiped at the change epoch recorded in that table (232 on the current PR branch). Exact epoch is still set by core when the approved code is scheduled; this proposal authorizes **RESET + the new logic**, not a promise that the number will never be adjusted in the release commit.

After RESET, `betaStartEpoch` is 0 until the next `BEGIN_EPOCH` anchors it. That is when the 14-epoch clock starts. Owner / operator setup that lived only in contract state must be re-established through the existing procedures. QST / QSC / STAR asset balances outside the wiped struct are not the RESET target.
...
player[Player] -->|coinFlip QSC or STAR| gate{Beta window and QST greater or equal 100k?}
gate -->|no| reject[QUSINO_INSUFFICIENT_QST_FOR_BETA]
gate -->|yes or beta over| qusino2[Qusino]
qusino2 --> settle[Draw mix and settle]
settle --> win[Win credits QSC or STAR]
settle --> loss[Loss keeps Qu in bank or burns STAR]
qst[QST holder] -->|membership / VIP / buy-sell / beta access| qusino
...
The PR also includes beta-gate cases (including `coinFlip_BetaGateAllowsCallerWithSufficientQST` and `getUserAssetVolume_ReportsQSTBalanceAndBetaStatus`).

**Additional coverage required for the QST / split / beta change** (same merge or a follow-on commit under this vote):
...
- during the 14-epoch window, `coinFlip` rejects callers with &lt; 100,000 QST via `QUSINO_INSUFFICIENT_QST_FOR_BETA`;
- after the window, `coinFlip` accepts callers with 0 QST;
- `getUserAssetVolume` `betaActive` / `betaEligible` / `betaEpochsRemaining` match the gate;
- QST balance is read from QX possession, not Qusino-managed shares.

PR author reports the Coin Flip / bank suite passing. Core reviewer (`fnordspace`) approved the integration **with the explicit note that core does not review contract game logic** and that the author must ensure the contract behaves as intended. Quorum approval is the governance step that authorizes shipping that logic, including the beta gate, QST dividend removal, the 50% SC-share split, and the Qusino **RESET**.
...
| Logic bugs in payout math | Unit tests; 1.96× payout constant is explicit; minimum bet set to 3 in final commits. |
...
| Legal residual risk | QST has no dividend; membership / beta access only. This is not a legal opinion. |
| RESET wipes live Qusino state | Documented; not PADDING. Operator must re-fund `bonusAmount` and re-set in-contract config after the change epoch. `betaStartEpoch` re-anchors on first `BEGIN_EPOCH`. |
| Beta gate reads 0 QST | Query QX as manager/possessor path, not `SELF_INDEX`. Covered by PR tests. |
| Users locked out of Coin Flip | Only for 14 epochs, and only below 100,000 QST. Status fields expose remaining epochs. |
...
5. **Beta duration is 14 epochs for eligible QST holders only.** After `betaStartEpoch` is set, `coinFlip` requires ≥ 100,000 QST possessed on QX until `betaStartEpoch + 14`. Then the gate expires. Failure code is `QUSINO_INSUFFICIENT_QST_FOR_BETA`.
6. **QST holder dividends are removed** so QST is not a revenue-share token in targeted jurisdictions. `END_EPOCH` must not pay Qu to QST possessors.
7. QST is authorized only as a **Membership / VIP** token (perks; 14-epoch beta access; sale/redemption inventory). That limitation is the stated reason for (6).
8. The former **30% QST slice is paid to Qusino SC shareholders**, making the shareholder share **50%**. LP 20%, CCF 5%, treasury 25% unchanged. Totals 100%.
9. No further scope (Blackjack, Baccarat, raw-Qu bets, new QST yield scheme, extending beta beyond 14 epochs) is authorized by this proposal.
...
- Beta constants on the PR branch: `QUSINO_BETA_DURATION_EPOCHS = 14`, `QUSINO_BETA_MIN_QST_HOLDING = 100000`
